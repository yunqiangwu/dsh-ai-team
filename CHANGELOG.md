...
- **停摆引导（P1-A）**：循环未启动（`stopped`）或暂停（`paused`）时面板补空状态提示——`stopped` 提示调 `autopilot_run` 启动、`paused` 提示调 `autopilot_resume` 恢复，复用 PhaseGuide 视觉，与阶段引导正交不互相覆盖。
- **浮窗无障碍闭环（P1-B）**：统计卡浮窗补 `role="dialog"` + `aria-modal` + `aria-label`，打开时焦点移入浮窗、`Esc` 关闭并归还焦点给触发的统计卡按钮，✕ 带可访问标签。
- **团队切换失败反馈（P1-C）**：多团队切换请求失败（网络 / 非 2xx）不再静默弹回，显示一句可见错误并 `aria-invalid`；成功路径无多余噪音。
- **看板过滤一键清除（P1-D）**：关键字/成员/周期任一过滤生效时工具栏出现「清除过滤」，一键复位三个条件；无条件生效时不显示。
