...
每个开发任务的 DoD 因此固定为四件，顺序不可调：契约里的 Gherkin → 同分支单测（先红后绿）→ `gates_run` 全绿 → 验收证据写进审查留言 / 任务单（本地合并，不建 PR）。
...
4. ~~多团队并行时 phase 是否提升为 team 级——本文已按 team 级设计，但要确认与 `activeTeamId` 的渲染假设一致~~ 已决并落代码（[`TECH-4`](../.tasks/TECH-4.md)）：phase 本就是 team 级、面板按 `activeTeamId`（缺省回退第一个团队）渲染单团队，假设一致；`activeTeamId` 的语义是「最后变更的团队」（`changed(teamId)` 抢占式更新），多团队时面板跟着最近活跃的团队走，属既定产品决策。顺手修掉的真缺口：`escalationView` / `deployView` 此前没有 `teamId`，多团队时升级流与部署历史全部混显——现已补字段（`stateVersion` 8）并由面板按当前团队过滤（`null` 的旧记录全团队可见，升级不能被过滤吞掉）。团队切换 UI 不做（超出本文范围，见首节「不在范围」）。
