...
在 `conversation.input.dock` 插槽渲染：运行状态灯（running/paused/escalated/completed/stopped）+ **阶段徽标**（非派发阶段用「等待」配色并显示「下一步」引导提示；标题栏另挂一个琥珀计数，聚合待答问卷、未解决升级、卡住任务与 needs-human 任务四类「需要人」的行动项，点开即**待办中心**浮窗内联作答）、八列看板（含 needs-human、needs-clarification 与 cancelled，挂着未答问卷的任务额外标一个**等人回答**；支持按关键字/成员/周期过滤，任务卡点击展开详情、方向键在卡片间移动焦点）、质量门徽标与 CI 徽标（CI 徽标仅 github 平台有数据，自建远端 generic 下无）、**等你决策**（未答问卷直接内联成表单）、**升级事件流**（未解除的升级同样内联一张 decision + note 表单，`suggestion` 可「按建议执行」一键预填）、问卷流水（只留历史：`已答复` / `已答复，等组长继续` / `已超时` / `已取消`）、部署历史、**已知教训**（按被印证次数排序，含已升格标记）、**卡住的任务**（前置无法满足的依赖）、**最近动态**（评审/升级/部署/问卷/教训按时间倒序的浮窗）。loop completed 时面板展示交付摘要；已答复待继续的 async 问卷显示醒目横幅并提示回会话说「继续」；右上角铃铛授权后新升级弹系统通知。多团队时面板顶部显示团队选择器，切换经同源路由 `POST /autopilot/team` 更新服务端 `activeTeamId` 并推新快照。数据流沿用 session 事件（`autopilot/update` 全量快照）+ 投影（last-write-wins），不引入 RPC。看板主体为 **CSS 多列瀑布流**（`columns` 断列，容器不跨栏断裂）：各状态列按分组紧凑排布、有内容的撑开、空列不占大片空白，未绑定任务的开放式问卷并入「等你决策」容器（计数只统计 `open` 且未绑定任务的）。看板主体高度默认不超过 `min(62vh, 720px)`、超出内部滚动，右下角有拖拽手柄可手动改高度（`resize: vertical`）。无人值守循环每拍有状态变更时主动向会话推一帧最新投影（空闲退避时不推），让面板的 in_progress / in_review 中间态与 state.json 对齐、不再滞拍。
