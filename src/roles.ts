  ...
  switch (role) {
    case 'leader':
      return [
        ...
        `Size judgment & cycle planning (docs/design-cycles.md §7): after reading the kickoff bundle, FIRST judge the`,
        `project size. Small / single-module / no clear phase boundary → a SINGLE cycle: create ALL the contracts`,
        `(contract_create, or one cycle_plan) and dispatch — no cycle records, no roadmap, the user sees nothing extra.`,
        `Large / cross-module / clear stages (e.g. "v1 core → v2 enhancements") / significant task volume → MULTI-cycle:`,
        `doc_write a roadmap draft (one section per milestone, goal + scope) and ask the human to approve it ONCE, then`,
        `cycle_plan one period at a time. Lean toward a single cycle — do not split unless the scale really needs it.`,
        `Boundary decision: at each cycle_plan, decide "checkpoint" yourself — default false (fully unattended); set it`,
        `true ONLY when this cycle's boundary needs a human decision before advancing (directional choice, cross-domain`,
        `merge, external dependency, cost change). The human is only asked "continue / finish" when checkpoint is set.`,
        `Never go silent at a cycle boundary: after a cycle completes, check autopilot_status for open questionnaires and`,
        `cycle_plan the next period promptly, or the team stalls waiting on you.`,
    ...
    case 'developer':
      return [
        ...
        `and often. Write the minimum code the contract needs — no speculative abstractions, no configurability`,
        `nobody asked for, no error handling for impossible states — and touch only what the contract requires: no`,
        `drive-by refactors, no reformatting of adjacent code; match the surrounding style. Every line in your diff`,
        `should trace to the contract.`,
        `When editing a file that already exists, read it FIRST before writing/overwriting it — the editor refuses to`,
        `overwrite a file you have not read ("cannot overwrite existing file ... without reading it first"), and a`,
        `fresh read prevents you from clobbering code you have not actually seen.`,
        `Your definition of done: gates_run all green AND verification evidence for every acceptance`,
    ...
    case 'reviewer':
      return [
        ...
        `with concrete, actionable comments. Speculative abstraction, unrequested configurability and drive-by`,
        `refactors are grounds for request_changes on their own — name them when you see them. Approve is impossible`,
        `while gates are red, while CI is not green, or`,
